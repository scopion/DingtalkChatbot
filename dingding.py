...
def getstatusphone(conn):
    mobiles = []
    sql = "SELECT phonenum FROM `accountcontacters` WHERE aegistatus = 0;"
    cursor = conn.cursor()
    try:
...
if __name__ == '__main__':
    # WebHook地址
    webhook = 'https://oapi.dingtalk.com/robot/send?access_token=12241899623'
    # 初始化机器人小丁
...
'''
dingding_url = "https://oapi.dingtalk.com/robot/send?access_token=12241825e65792" # 这个 url 从 PC 端钉钉群组->管理机器人里获得
...
for i in list:
    ...
    post_data = {
        ...
        "at": {
            "atMobiles": [phonenum，sdfwe,13566,1475822],
    		"isAtAll": false
        }
    }
